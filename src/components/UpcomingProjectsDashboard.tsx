...
export function UpcomingProjectsDashboard({ projects, userEmail }: UpcomingProjectsDashboardProps) {
  ...
  const triggerDownloadSimulation = (fileName: string) => {
    alert(`Downloading corporate proposal file: "${fileName}".`);
  };
              ...
              {projects.length === 0 && (
                <div className="col-span-full text-center py-12 bg-slate-50/50 rounded-2xl border border-slate-150 p-6">
                  ...
                  <p className="text-[10px] text-slate-350 mt-1 max-w-sm mx-auto">
                    Toggle to the Administrative Module and choose the "Projects" tab to construct or AI-hydrate potential pilots.
                  </p>
                </div>
              )}
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {selectedProj.documents?.map((item, idx) => (
                    ...
                    >
                      ...
                      <button
                        type="button"
                        onClick={() => triggerDownloadSimulation(item.name)}
