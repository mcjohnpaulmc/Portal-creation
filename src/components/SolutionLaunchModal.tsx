...
import { X, ExternalLink, Copy, Check, Shield, User, Key, Info } from "lucide-react";
          ...
          <div className="bg-slate-950 text-slate-200 p-4.5 rounded-xl space-y-3.5 font-mono text-xs border border-slate-900 shadow-inner">
            <div className="flex items-center justify-between text-[10px] tracking-widest text-slate-500 uppercase pb-2 border-b border-slate-800">
              <span className="flex items-center gap-1.5">
                <Shield className="h-3 w-3 text-slate-400" /> Authorized Pipeline Credentials
              </span>
              <span>Secure Prefill Active</span>
            </div>
            ...
            {solution.usernamePrefill && (
              <div className="flex items-center justify-between gap-4 py-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-slate-200 selection:bg-slate-700">{solution.usernamePrefill}</span>
                </div>
                <button
                  type="button"
                  onClick={() => handleCopy(solution.usernamePrefill || "", "username")}
                  className="p-1.5 hover:bg-slate-800 text-slate-400 hover:text-slate-200 rounded-md transition-colors shrink-0"
                  title="Copy Username"
                >
            ...
            {solution.passwordPrefill && (
              <div className="flex items-center justify-between gap-4 py-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-slate-200 tracking-wider font-semibold selection:bg-slate-700">{solution.passwordPrefill}</span>
                </div>
                <button
                  type="button"
